...
async function boot(){
 ...
 const finder=()=>{if(!result)return;const n=Number(!!state.audience)+Number(!!state.need);result.classList.toggle('is-ready',n===2);result.querySelector('small').textContent=`${n} / 2`;if(n<2){result.querySelector('strong').textContent=n===1?'Perfetto. Ora scegli cosa cerchi.':'Completa le due scelte.';result.querySelector('#finderSelection')?.replaceChildren();return}const arr=find();result.querySelector('strong').textContent=arr.length?`Ecco cosa abbiamo scelto per te.`:'Non lo vediamo nell’Online Edit. Il team può cercarlo in store.';const sel=result.querySelector('#finderSelection');if(sel){sel.innerHTML=arr.length?`<div class="f29-finder-picked">${arr.slice(0,6).map(card).join('')}</div><a class="f29-finder-more" href="collections.html#selection">VIEW THE FULL EDIT ↘</a>`:`<div class="f29-finder-empty">Prova un'altra combinazione oppure chiedi al team Flip&Co.</div>`;}setTimeout(()=>result.scrollIntoView({behavior:'smooth',block:'start'}),80)};
  ...
  function bootShopTheLook(){
    ...
    function renderProduct(id){
      ...
      const sale=p.compareAt&&Number(p.compareAt)>Number(p.price)
        ...
        :esc(money(p.price));
      box.innerHTML=`<article class="flip-look-product-card">
        <img src="${esc(image)}" alt="${esc(`${p.brand||""} ${p.name||""}`)}" loading="lazy" onerror="this.onerror=null;this.src='${esc(art)}'">
        ...
        <a class="flip-look-product-link" href="product.html?id=${encodeURIComponent(p.id)}">Scopri il pezzo ↗</a>
      </article>`;
    }
